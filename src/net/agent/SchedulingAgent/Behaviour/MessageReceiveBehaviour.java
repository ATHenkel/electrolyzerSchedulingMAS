...
public class MessageReceiveBehaviour extends CyclicBehaviour {
	...
	public void action() {
		...
		if (this.schedulingAgent.getInternalDataModel().isReceiveMessages()) {
			...
			/**
			 * Processes received ACLMessage, extracting content and performing actions
			...
			if (receivedMsg != null) {
				...
				// Messages from Broker-Agent
				if ("StartOptimization".equals(content)) {
					System.out.println("PEA-Agent:" + this.schedulingAgent.getLocalName()
							+ " start Optimization message received");
					this.schedulingAgent.getInternalDataModel().setStartOptimization(true);
			        MinimizeX minimizeX = new MinimizeX(schedulingAgent);
			        schedulingAgent.addBehaviour(minimizeX);
				} else {
					// Messages from other PEA-agents
					...
					if (this.schedulingAgent.getInternalDataModel().getCountReceivedMessages() == 0) {
						...
						this.schedulingAgent.getInternalDataModel().setSumProduction(msgProductionQuantity); // Add Production Quantity to internal knowledgeBase
