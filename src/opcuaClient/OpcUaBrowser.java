...
public class OpcUaBrowser {
	...
	/**
	 * Finds the complete nodeId for a given partial nodeId that contains the
	 * browseName
	 * 
	...
	public NodeId findNodeId(String nodeIdWithPlaceholder) throws NodeIdResolvingException {
		...
		// BIG Assumption: localVarName (last array entry) is used as browseName - this
		// is valid for Codesys, but also for other tools?
		...
		List<TreeNode<ReferenceDescription>> matchingRefs = flatNodeList.stream().filter(isMatchingRef(localVarName))
	...
	/**
	 * Wrapper function for a predicate to check whether a reference's browseName
	 * matches a given local variable name
	 * 
	...
	/**
	 * By comparing only the browseNames, multiple matches may be found. Thus, we
	 * need to compare their parents (representing the POU) to get the "real" match
	 * 
	 * @param nodeIdElements String array containing a partial nodeId that consists
	 *                       of a template string, the application structure (with
	 *                       the POUs) and the local var name
	 * @param matchingRefs   A list of matching reference descriptions after
	 *                       comparing only their local variable name
	 * @return
	 * @throws NodeIdResolvingException
	 */
	private ExpandedNodeId findCorrectMatchingRef(String[] nodeIdElements,
			List<TreeNode<ReferenceDescription>> matchingRefs) throws NodeIdResolvingException {
		...
		if (correctMatches.size() == 0) {
			...
			throw new NodeIdResolvingException("No matches for the unresolved nodeId " + unresolvedNodeId
					+ ". Please resolve this nodeId manually");
		}

		// Browsing can return multiple referenceDescriptions to one node. In the end,
		// we need to return one nodeId -> correctMatches need to be made unique
		Set<ExpandedNodeId> correctMatchNodeIds = correctMatches.stream()
				.map(correctMatch -> correctMatch.getData().getNodeId()).collect(Collectors.toSet());

		// There should be exactly one correct matches. If there are more or none, we
		// can only throw an error
		if (correctMatchNodeIds.size() > 1) {
			String unresolvedNodeId = String.join(".", nodeIdElements);
			throw new NodeIdResolvingException(
					"There are '" + correctMatchNodeIds.size() + "' matches for the unresolved nodeId "
							+ unresolvedNodeId + ". Please resolve this nodeId manually");
		}
